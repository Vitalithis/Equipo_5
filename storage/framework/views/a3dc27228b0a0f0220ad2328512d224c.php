...
<div class="max-w-6xl mx-auto px-6 py-10 bg-white rounded-2xl shadow-xl">
    <h2 class="text-4xl font-extrabold text-eprimary mb-8 text-center">Editar Pedido #<?php echo e($pedido->id); ?></h2>
        <div class="mb-6">
        <a href="<?php echo e(route('pedidos.index')); ?>" class="inline-flex items-center px-4 py-2 bg-gray-100 hover:bg-gray-200 text-eprimary font-semibold rounded-lg shadow-sm transition">
            ← Volver a la lista de pedidos

        </a>

    </div>
    <form action="<?php echo e(route('pedidos.update', $pedido->id)); ?>" method="POST" id="pedido-form">
        <?php echo csrf_field(); ?>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="metodo_entrega" id="metodo-entrega" class="mt-1 block w-full rounded-lg border border-gray-300 bg-white py-2 px-4 shadow-sm focus:border-eprimary focus:ring focus:ring-eprimary/50" required>
                    <option value="retiro" <?php echo e($pedido->metodo_entrega == 'retiro' ? 'selected' : ''); ?>>Retiro en tienda</option>
                    <option value="domicilio" <?php echo e($pedido->metodo_entrega == 'domicilio' ? 'selected' : ''); ?>>Entrega a domicilio</option>
                </select>
            </div>
        ...
        <div id="direccion-contenedor" class="mt-6 <?php echo e($pedido->metodo_entrega == 'domicilio' ? '' : 'hidden'); ?>">
            <label class="block text-sm font-semibold text-gray-700 mb-1">Dirección de entrega</label>
            <textarea name="direccion_entrega" rows="3" class="w-full rounded-lg border border-gray-300 py-2 px-4 shadow-sm focus:border-eprimary focus:ring focus:ring-eprimary/50" placeholder="Ej: Calle 123, Depto 4B, Comuna..."><?php echo e($pedido->direccion_entrega); ?></textarea>
        </div>

        <!-- Código de descuento -->
        <div class="mt-6">
            <label class="block text-sm font-semibold text-gray-700 mb-1">Descuento (%)</label>
            <input type="number" name="descuento_porcentaje" min="0" max="100" step="0.01" value="<?php echo e(round(($pedido->descuento / max($pedido->subtotal,1)) * 100, 2)); ?>" class="w-full rounded-lg border border-gray-300 py-2 px-4 shadow-sm focus:border-eprimary focus:ring focus:ring-eprimary/50" placeholder="Ej: 10">
        ...
        <div class="mt-6">
            ...
            <select name="forma_pago" class="mt-1 block w-full rounded-lg border border-gray-300 bg-white py-2 px-4 shadow-sm focus:border-eprimary focus:ring focus:ring-eprimary/50" required>
                <option value="efectivo" <?php echo e($pedido->forma_pago == 'efectivo' ? 'selected' : ''); ?>>Efectivo</option>
                <option value="tarjeta" <?php echo e($pedido->forma_pago == 'tarjeta' ? 'selected' : ''); ?>>Tarjeta</option>
                <option value="transferencia" <?php echo e($pedido->forma_pago == 'transferencia' ? 'selected' : ''); ?>>Transferencia</option>
            </select>
        </div>
        ...
        <div class="mt-6">
            ...
            <select name="estado_pago" class="mt-1 block w-full rounded-lg border border-gray-300 bg-white py-2 px-4 shadow-sm focus:border-eprimary focus:ring focus:ring-eprimary/50" required>
                ...
                <option value="parcial" <?php echo e($pedido->estado_pago == 'parcial' ? 'selected' : ''); ?>>Parcial</option>
                <option value="reembolsado" <?php echo e($pedido->estado_pago == 'reembolsado' ? 'selected' : ''); ?>>Reembolsado</option>
            </select>
        </div>
        ...
        <div class="mt-6">
            ...
            <input type="number" step="0.01" name="monto_pagado" value="<?php echo e($pedido->monto_pagado); ?>" class="w-full rounded-lg border border-gray-300 py-2 px-4 shadow-sm focus:border-eprimary focus:ring focus:ring-eprimary/50">
        </div>
        ...
        <div class="mt-6">
            ...
            <select name="tipo_documento" class="mt-1 block w-full rounded-lg border border-gray-300 bg-white py-2 px-4 shadow-sm focus:border-eprimary focus:ring focus:ring-eprimary/50" required>
                <option value="boleta" <?php echo e($pedido->tipo_documento == 'boleta' ? 'selected' : ''); ?>>Boleta</option>
                <option value="factura" <?php echo e($pedido->tipo_documento == 'factura' ? 'selected' : ''); ?>>Factura</option>
            </select>
        </div>
        ...
        <div class="mt-6">
            ...
            <textarea name="observaciones" rows="3" class="w-full rounded-lg border border-gray-300 py-2 px-4 shadow-sm focus:border-eprimary focus:ring focus:ring-eprimary/50" placeholder="Opcional"><?php echo e($pedido->observaciones); ?></textarea>
        </div>
        ...
        <div id="detalle-container" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end relative">
                <div class="relative">
                    ...
                    <input type="text" class="producto-nombre w-full rounded-md border-gray-300 shadow-sm" placeholder="Buscar producto..." autocomplete="off" value="<?php echo e($detalle->nombre_producto_snapshot); ?>" required>
                    <input type="hidden" name="producto_id[]" class="producto-id" value="<?php echo e($detalle->producto_id); ?>">
                ...
                <div>
                    ...
                    <input type="number" name="cantidad[]" class="cantidad-input w-full rounded-md border-gray-300 shadow-sm" min="1" value="<?php echo e($detalle->cantidad); ?>" required>
                </div>

                <div>
                    <label class="text-sm text-gray-700">Precio Unitario</label>
                    <input type="number" name="precio_unitario[]" step="0.01" class="precio-input w-full rounded-md border-gray-300 shadow-sm bg-gray-100" value="<?php echo e($detalle->precio_unitario); ?>" readonly>
                </div>

                <div>
                    <label class="text-sm text-gray-700">Subtotal</label>
                    <input type="number" step="0.01" class="subtotal-input w-full rounded-md border-gray-300 shadow-sm bg-gray-100" value="<?php echo e($detalle->subtotal); ?>" readonly>
                </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.getElementById('pedido-form').addEventListener('submit', function(e) {
            const ids = document.querySelectorAll('.producto-id');
            for (let id of ids) {
                if (!id.value) {
                    alert('Debes seleccionar un producto desde la lista sugerida, no solo escribir el nombre.');
                    e.preventDefault();
                    return;
                }
            }
...
<?php echo $__env->make('dashboard', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\xampp\Equipo_5\resources\views/pedidos/partials/edit.blade.php ENDPATH**/ ?>