@php
    ...
    $navbarColor = $pref?->navbar_color ?? '#FFFFF';
...
<div class="relative hidden md:block" x-data="{ open: false }">
    @auth
        ...
        <div x-show="open" @click.outside="open = false"
             class="absolute right-0 mt-2 w-52 rounded-md shadow-lg z-50"
            ...
            <a href="{{ route('cotizacion.index') }}" class="block px-4 py-2 hover:bg-opacity-20">Mis Cotizaciones</a>
            <a href="{{ route('cart.index') }}" class="block px-4 py-2 hover:bg-opacity-20">Mi Carrito</a>
            <a href="{{ route('compras.index') }}" class="block px-4 py-2 hover:bg-opacity-20">Mis Pedidos</a>
...
<div x-show="menuOpen" x-transition class="md:hidden bg-[var(--navbar-color)] px-6 pt-4 pb-6 space-y-4 shadow-md" style="color: {{ $navbarTextColor }}" x-cloak>
    <ul class="space-y-2">
        ...
        @else
            ...
            @else
                <li><a href="{{ route('login') }}" class="block hover:opacity-80 text-lg" style="color: {{ $navbarTextColor }}">Iniciar sesión</a></li>
            @endauth
        @endif
    </ul>
</div>
