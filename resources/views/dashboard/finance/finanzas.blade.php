...
<div class="py-8 px-4 md:px-8 w-full font-['Roboto'] text-gray-800">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        ...
        <form method="GET" action="{{ route('dashboard.finanzas') }}" class="flex flex-wrap items-center gap-4">
            ...
            <button type="submit"
                class="px-4 py-2 border border-green-600 text-green-700 hover:bg-green-100 rounded transition text-sm">
                Filtrar
            </button>
            @if(request()->filled('desde') || request()->filled('hasta'))
                <a href="{{ route('dashboard.finanzas') }}"
                   class="px-4 py-2 border border-gray-300 text-gray-700 hover:bg-gray-100 rounded transition text-sm">
                    Limpiar
                </a>
            @endif
        </form>
        ...
        <a href="{{ route('finanzas.create') }}"
           class="flex items-center text-green-700 hover:text-green-800 border border-green-700 hover:border-green-800 px-3 py-2 rounded transition-colors text-sm">
