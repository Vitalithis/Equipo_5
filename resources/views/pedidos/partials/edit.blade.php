...
<div class="max-w-6xl mx-auto px-6 py-10 bg-white rounded-2xl shadow-xl">
    <h2 class="text-4xl font-extrabold text-eprimary mb-8 text-center">Editar Pedido #{{ $pedido->id }}</h2>
        <div class="mb-6">
    ...
    <form action="{{ route('pedidos.update', $pedido->id) }}" method="POST" id="pedido-form">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="metodo_entrega" id="metodo-entrega" class="mt-1 block w-full rounded-lg border border-gray-300 bg-white py-2 px-4 shadow-sm focus:border-eprimary focus:ring focus:ring-eprimary/50" required>
                    <option value="retiro" {{ $pedido->metodo_entrega == 'retiro' ? 'selected' : '' }}>Retiro en tienda</option>
                    <option value="domicilio" {{ $pedido->metodo_entrega == 'domicilio' ? 'selected' : '' }}>Entrega a domicilio</option>
                </select>
            </div>
        ...
        <div id="direccion-contenedor" class="mt-6 {{ $pedido->metodo_entrega == 'domicilio' ? '' : 'hidden' }}">
            <label class="block text-sm font-semibold text-gray-700 mb-1">Dirección de entrega</label>
            <textarea name="direccion_entrega" rows="3" class="w-full rounded-lg border border-gray-300 py-2 px-4 shadow-sm focus:border-eprimary focus:ring focus:ring-eprimary/50" placeholder="Ej: Calle 123, Depto 4B, Comuna...">{{ $pedido->direccion_entrega }}</textarea>
        </div>

        <!-- Código de descuento -->
        <div class="mt-6">
            <label class="block text-sm font-semibold text-gray-700 mb-1">Descuento (%)</label>
            <input type="number" name="descuento_porcentaje" min="0" max="100" step="0.01" value="{{ round(($pedido->descuento / max($pedido->subtotal,1)) * 100, 2) }}" class="w-full rounded-lg border border-gray-300 py-2 px-4 shadow-sm focus:border-eprimary focus:ring focus:ring-eprimary/50" placeholder="Ej: 10">
        ...
        <div class="mt-6">
            ...
            <select name="estado_pago" class="mt-1 block w-full rounded-lg border border-gray-300 bg-white py-2 px-4 shadow-sm focus:border-eprimary focus:ring focus:ring-eprimary/50" required>
                ...
                <option value="parcial" {{ $pedido->estado_pago == 'parcial' ? 'selected' : '' }}>Parcial</option>
        ...
        <div class="mt-6">
            ...
            <input type="number" step="0.01" name="monto_pagado" value="{{ $pedido->monto_pagado }}" class="w-full rounded-lg border border-gray-300 py-2 px-4 shadow-sm focus:border-eprimary focus:ring focus:ring-eprimary/50">
        </div>
        ...
        <div class="mt-6">
            ...
            <select name="tipo_documento" class="mt-1 block w-full rounded-lg border border-gray-300 bg-white py-2 px-4 shadow-sm focus:border-eprimary focus:ring focus:ring-eprimary/50" required>
                <option value="boleta" {{ $pedido->tipo_documento == 'boleta' ? 'selected' : '' }}>Boleta</option>
        ...
        <div class="mt-6">
            ...
            <textarea name="observaciones" rows="3" class="w-full rounded-lg border border-gray-300 py-2 px-4 shadow-sm focus:border-eprimary focus:ring focus:ring-eprimary/50" placeholder="Opcional">{{ $pedido->observaciones }}</textarea>
        </div>
        ...
        <div id="detalle-container" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end relative">
                <div class="relative">
                    ...
                    <input type="text" class="producto-nombre w-full rounded-md border-gray-300 shadow-sm" placeholder="Buscar producto..." autocomplete="off" value="{{ $detalle->nombre_producto_snapshot }}" required>
                    <input type="hidden" name="producto_id[]" class="producto-id" value="{{ $detalle->producto_id }}">
                ...
                <div>
                    ...
                    <input type="number" name="cantidad[]" class="cantidad-input w-full rounded-md border-gray-300 shadow-sm" min="1" value="{{ $detalle->cantidad }}" required>
                </div>

                <div>
                    <label class="text-sm text-gray-700">Precio Unitario</label>
                    <input type="number" name="precio_unitario[]" step="0.01" class="precio-input w-full rounded-md border-gray-300 shadow-sm bg-gray-100" value="{{ $detalle->precio_unitario }}" readonly>
                </div>

                <div>
                    <label class="text-sm text-gray-700">Subtotal</label>
                    <input type="number" step="0.01" class="subtotal-input w-full rounded-md border-gray-300 shadow-sm bg-gray-100" value="{{ $detalle->subtotal }}" readonly>
                </div>
