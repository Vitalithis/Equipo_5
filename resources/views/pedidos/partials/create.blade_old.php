...
<form action="{{ isset($pedido->id) ? route('pedidos.update', $pedido->id) : route('pedidos.store') }}" method="POST" id="pedido-form" class="space-y-6">
    @csrf
    @if(isset($pedido->id))
        @method('PUT')
    @endif
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <!-- Aquí agregamos la parte de la dirección -->
        <div id="direccion-contenedor" class="mt-8 {{ (old('metodo_entrega', $pedido->metodo_entrega ?? '') == 'domicilio') ? '' : 'hidden' }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <input type="text" name="ciudad" placeholder="Ciudad" required
                    ...
                    class="w-full md:col-span-2 px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all">
            </div>
        </div>
        <div>
            <label for="tipo_documento" class="mt-8 block text-sm font-medium text-gray-700 mb-1">Tipo de Documento</label>
        ...
        <div>
            <label for="forma_pago" class="mt-8 block text-sm font-medium text-gray-700 mb-1">Forma de Pago</label>
        ...
        <div>
            <label for="estado_pago" class="mt-8 block text-sm font-medium text-gray-700 mb-1">Estado del Pago</label>
            <select name="estado_pago"
                ...
                <option value="parcial" {{ old('estado_pago', $pedido->estado_pago ?? '') == 'parcial' ? 'selected' : '' }}>Parcial</option>
                <option value="reembolsado" {{ old('estado_pago', $pedido->estado_pago ?? '') == 'reembolsado' ? 'selected' : '' }}>Reembolsado</option>
            </select>
        </div>

        <div>
            <label for="monto_pagado" class="mt-8 block text-sm font-medium text-gray-700 mb-1">Monto Pagado</label>
            <input type="number" step="0.01" name="monto_pagado"
                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all"
                value="{{ old('monto_pagado', $pedido->monto_pagado ?? '') }}">
        </div>

        <div>
            <label for="descuento_porcentaje" class="mt-8 block text-sm font-medium text-gray-700 mb-1">Descuento (%)</label>
        ...
        <div class="grid grid-cols-1 md:grid gap-6 mt-8">
                <label for="metodo-entrega" class="block text-sm font-medium text-gray-700 mb-1">
                        Detalle del pedido <span class="text-red-500">*</span>
                </label>
                ...
                @if(isset($pedido) && $pedido->detalles->count() > 0)
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end relative">
                        <div class="relative">
                            ...
                            <input type="text" name="producto_nombre[]" class="producto-nombre w-full rounded-md border border-gray-300 shadow-sm"
...
<div class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end relative">
    ...
    <div>
        ...
        <input type="number" name="precio_unitario[]" step="0.01"
            class="precio-input w-full rounded-md border border-gray-300 shadow-sm bg-gray-100" readonly
            value="{{ number_format($productos[0]->precio ?? 0, 2, '.', '') }}">
    </div>
    ...
    <div class="text-right mt-4 space-y-1">
        <div><span class="font-semibold text-lg">Subtotal:</span> $<span id="subtotal-dinamico">0.00</span></div>
        <div><span class="font-semibold text-lg">Descuento:</span> $<span id="descuento-dinamico">0.00</span></div>
        <div><span class="font-semibold text-lg">Total:</span> $<span id="total-dinamico">0.00</span></div>
    </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function bindEvents(row) {
            ...
            if (document.querySelectorAll('#detalle-container .remove-row').length <= 1) {
        ...
        document.getElementById('add-detalle').addEventListener('click', () => {
            ...
            if (metodoSelect.value !== 'domicilio') {
                direccionContenedor.querySelectorAll('[required]').forEach(input => {
    ...
    function calcularSubtotalDinamico() {
        let subtotal = 0;
        document.querySelectorAll('#detalle-container > div.grid').forEach(row => {
        ...
        let descuentoPorcentaje = parseFloat(descuentoPorcentajeInput ? descuentoPorcentajeInput.value : 0) || 0;

        let descuento = subtotal * (descuentoPorcentaje / 100);
        let total = subtotal - descuento;

        document.getElementById('subtotal-dinamico').textContent = subtotal.toFixed(2);
        document.getElementById('descuento-dinamico').textContent = descuento.toFixed(2);
        document.getElementById('total-dinamico').textContent = total.toFixed(2);
    }

    document.getElementById('detalle-container').addEventListener('input', (e) => {
        if(e.target.classList.contains('cantidad-input') || e.target.classList.contains('precio-input') || e.target.name === 'descuento_porcentaje'){
            calcularSubtotalDinamico();
        }
    });

    document.getElementById('detalle-container').addEventListener('click', (e) => {
        if(e.target.classList.contains('remove-row')){
            e.target.closest('.grid').remove();
            calcularSubtotalDinamico();
            const removeButtons = document.querySelectorAll('#detalle-container .remove-row');
...
document.getElementById('pedido-form').addEventListener('submit', function (e) {
    ...
    // Si se eligió "retiro", desactivar temporalmente los campos requeridos ocultos
    ...
    // Validar que todos los productos tengan su ID seleccionado
    let valido = true;
    document.querySelectorAll('.producto-id').forEach(id => {
        if (!id.value) {
            valido = false;
        }
    });
