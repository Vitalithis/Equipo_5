        ...
        <div class="w-full h-[70vh] overflow-hidden relative">
            ...
            <div id="slider" class="flex h-full transition-transform duration-700">
                ...
                <div class="w-full flex-shrink-0 flex items-center justify-center">
                    <img src="{{ asset('/storage/images/slide1.jpg') }}" alt="Slide 1"
                        class="w-full h-full object-cover opacity-70 mix-blend-darken brightness-50">
                </div>

                <!-- Slider Position 2 -->
                <div class="w-full flex-shrink-0 flex items-center justify-center">
                    <img src="{{ asset('/storage/images/slide2.jpg') }}" alt="Slide 2"
                        class="w-full h-full object-cover opacity-70 mix-blend-darken brightness-50">
                </div>

                <!-- Slider Position 3 -->
                <div class="w-full flex-shrink-0 flex items-center justify-center">
                    <img src="{{ asset('/storage/images/slide3.jpg') }}" alt="Slide 3"
                        class="w-full h-full object-cover opacity-70 mix-blend-darken brightness-50">
                </div>
            </div>
    ...
    <section class="bg-gray-50 ">
        <div class="container mx-auto py-20">
            ...
            <div class="grid grid-cols-4 gap-4">
                @foreach ($productos->sortByDesc('created_at')->take(4) as $producto)
                    <div class="bg-white p-4 shadow rounded hover:p-2 hover:shadow-lg transition-all duration-300 hover:bg-eaccent"
                        ...
                        <a href="{{ route('products.show', $producto->slug) }}">
                            <!-- Imagen del producto -->
                            <img src="/storage/images/default-logo.png" alt="{{ $producto->nombre }}"
                            ...
                            <p class="text-sm text-gray-600">{{ number_format($producto->precio, 0, ',', '.') }} CLP</p>

                            @if ($producto->descripcion)
                                <p class="text-sm text-gray-600">{{ Str::limit($producto->descripcion, 80) }}</p>
                            @endif
                        </a>
                    </div>
                @endforeach
    ...
    <section class="container mx-auto py-8">
        <div class="grid grid-cols-4 gap-4">
            ...
            <aside class="col-span-1 bg-eaccent2-100 p-4 rounded sticky top-4 self-start">
                <form action="{{ route('products.index') }}#product-catalog" method="GET">
                        ...
                        <div class="text-sm text-efore border-b-4 border-green- rounded-sm">
                            <ul class="px-4 py-2">
                                @foreach($productos->unique('nivel_dificultad') as $producto)
                                    @if(!empty($producto->nivel_dificultad))
                                        <li class="flex items-center gap-2 text-base py-1">
                                            <input type="checkbox" id="dificultad_{{ $loop->index }}" name="dificultades[]"
                                                value="{{ $producto->nivel_dificultad }}"
                                                class="rounded-sm border border-gray-300 bg-white checked:border-eprimary-100 checked:bg-eprimary-100">
                                            <label for="dificultad_{{ $loop->index }}">{{ $producto->nivel_dificultad }}</label>
                                        </li>
                                    @endif
                                @endforeach
                            </ul>
